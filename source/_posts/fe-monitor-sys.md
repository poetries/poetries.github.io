---
title: 前端监控系统从 0 到 1 搭建全流程总结
description: 从指标采集、SDK 设计、上报策略到 ES 存储、告警与看板，完整拆解前端监控系统的搭建过程，含首屏、白屏、卡顿采集代码与上线 checklist。
...
![前端性能优化方法论整体框架](https://blog.poetries.top/img/static/images/20210502210714.png)
...
![性能指标与优化手段的对应关系](https://blog.poetries.top/img/static/images/20210502214539.png)

![性能优化方法论的落地路径](https://blog.poetries.top/img/static/images/20210503212926.png)
...
![首屏时间采集流程与关键节点](https://blog.poetries.top/img/static/images/20210503151137.png)
...
![卡顿指标采集与判定逻辑](https://blog.poetries.top/img/static/images/20210503152718.png)
...
![性能 SDK 的模块划分与 API 封装结构](https://blog.poetries.top/img/static/images/20210503152937.png)
...
| `navigator.sendBeacon` | 需要服务端 CORS | 高，浏览器保证发出 | 可带 body | 页面卸载前的兜底上报 |
| `fetch` / XHR POST | 需要服务端 CORS | 低，卸载时容易被 kill | 大 | 复杂结构、批量数据 |
...
![上报策略的判定流程](https://blog.poetries.top/img/static/images/20210503153920.png)
...
![性能平台大盘页效果，展示首屏时间、秒开率和采样 PV](https://blog.poetries.top/img/static/images/20210503154327.png)

大盘页只放三个数是有讲究的。首屏时间看绝对水平，秒开率看达标比例，采样 PV 看这份数据可不可信。少了第三个数，前面两个随时可能因为样本太少而失真，我见过采样 PV 只有几十的页面顶着一个非常漂亮的秒开率，点进去一看是内部测试流量。

那么，我们该如何搭建这样一个性能平台呢？

![前端性能平台技术架构图，自下而上分为接入层、计算层、存储层和平台层](https://blog.poetries.top/img/static/images/20210503154342.png)
...
![性能平台详情页，展示秒开率、性能均值与终端分布](https://blog.poetries.top/img/static/images/20210503160942.png)
...
![页面加载瀑布流图，把首屏时间拆分到各个网络阶段](https://blog.poetries.top/img/static/images/20210503161021.png)
...
![性能平台前后端技术选型与交互方式](https://blog.poetries.top/img/static/images/20210503161251.png)
...
另外一定要做告警收敛。同一个错误在一分钟内触发一千次，你要发的是一条「该错误 1 分钟内出现 1000 次」，而不是一千条通知。实现上就是按「错误指纹」聚合，指纹一般取错误类型 + 消息 + 堆栈首帧的哈希。这块 Sentry 做得很成熟，可以参考它的分组逻辑，我之前整理过一篇 [Sentry 的接入和用法总结](https://feinterview.poetries.top/blog/sentry-summary)，想直接用现成方案的可以看看。
...
![问题诊断的排查路径与判定标准](https://blog.poetries.top/img/static/images/20210503161715.png)
...
所以更好的组合是构建时给文件名加内容哈希（`app.8f3c2a.js` 这种），然后对带哈希的文件统统上强缓存 `Cache-Control: max-age=31536000, immutable`，只让入口 HTML 走协商缓存。内容变了文件名就变，天然不存在缓存不更新的问题，也不需要每次都去问服务器。这套现在基本是 Webpack、Vite 的默认输出形态，前端这边几乎不用额外做什么，只要运维侧的 nginx 规则配对就行。资源相关的更多优化手段，我在 [前端性能优化的实战总结](https://feinterview.poetries.top/blog/fe-youhua) 里写得更细一些。
...
![HTTP 2.0 多路复用与并行化优化示意](https://blog.poetries.top/img/static/images/20210503162414.png)
...
顺着上面聊，内存这条线也和卡顿有关，长时间不释放的监听器和闭包会让页面越用越卡，这块我在 [JS 内存泄漏的排查方法](https://feinterview.poetries.top/blog/js-memory-leak) 里单独写过。
...
![卡顿治理的两类成因与对应手段](https://blog.poetries.top/img/static/images/20210503163046.png)

到这里，性能这条线从采集到优化就走完了。下面换一条线，聊错误监控和一个更完整的 SDK 设计。

## 十、JS SDK 设计

![JS SDK 的整体能力设计](https://blog.poetries.top/img/static/images/20210503170316.png)
...
![Chrome Console 中 performance.timing 的输出](https://blog.poetries.top/img/static/images/20210503170506.png)
...
- `domLoading`：浏览器把`document.readyState`设置为`loading`的时间点，也就是开始构建`dom`树的时间点；
...
![performance.timing 各阶段的开始与结束时间点示意](https://blog.poetries.top/img/static/images/20210503170539.png)
...
![performance.getEntries() 返回的资源加载数组](https://blog.poetries.top/img/static/images/20210503170603.png)
...
- `jsHeapSizeLimit`：内存大小上限
- `totalJSHeapSize`：当前已分配的堆总量
- `usedJSHeapSize`：已使用的内存

> `usedJSHeapSize`表示所有被使用的JS堆内存，`totalJSHeapSize`是当前已分配的JS堆总量，如果`usedJSHeapSize`持续逼近`jsHeapSizeLimit`且一直不回落，就可能出现内存泄漏

![performance.memory 在 Chrome Console 中的输出](https://blog.poetries.top/img/static/images/20210503170616.png)
...
window.addEventListener('unhandledrejection', (event) => {
  report({
    ...
    // reason 可能是 Error，也可能是任意值，得兼容处理
...
window.addEventListener('error', (event) => {
  ...
  // 有 target.src 或 target.href 的才是资源加载错误
  if (!target || !(target.src || target.href)) return;
  report({
    type: 'resourceError',
...
![错误监控三条线的整体结构](https://blog.poetries.top/img/static/images/20210503175341.png)
...
![ELK 日志分析系统的数据流转](https://blog.poetries.top/img/static/images/20210503175506.png)
...
![基于 ES restful 接口自建的数据展示页](https://blog.poetries.top/img/static/images/20210503175531.png)
...
![没有监控时的常见线上痛点](https://blog.poetries.top/img/static/images/20210503175936.png)

### 手动搭建一个前端监控系统

在搭建前端监控系统开始之前，我们先来了解下整个系统的架构是怎么样的。因为你只有了解到流程是怎样走的，才清楚每个步骤我们该做些什么事情。

![手动搭建前端监控系统的整体架构](https://blog.poetries.top/img/static/images/20210503180034.png)
...
![上报上来的压缩后 JS 错误信息与堆栈](https://blog.poetries.top/img/static/images/20210503180112.png)
...
![通过 sourcemap 还原后的源码级错误位置](https://blog.poetries.top/img/static/images/20210503180234.png)
...
![代理 XMLHttpRequest 实现请求劫持](https://blog.poetries.top/img/static/images/20210503180304.png)
...
![接口上报采集的字段示例](https://blog.poetries.top/img/static/images/20210503180352.png)
...
![上报后按各阶段耗时拆解的图表](https://blog.poetries.top/img/static/images/20210503180431.png)

**关键性能指标图**

![关键性能指标的汇总展示](https://blog.poetries.top/img/static/images/20210503180455.png)
...
- SDK 上报采用哪种方式比较好。一般来说采用 `get`（`image` 方式上报，因为它避开了跨域限制，目前也是业界主流的上报方式）
...
![Node 服务接收上报数据并写入日志](https://blog.poetries.top/img/static/images/20210503180707.png)
...
![Logstash 用 geoip 和 useragent 插件解析后的字段](https://blog.poetries.top/img/static/images/20210503180802.png)
...
![用 es-head 插件查看 ES 里的数据](https://blog.poetries.top/img/static/images/20210503180831.png)
...
![告警规则配置的后台界面](https://blog.poetries.top/img/static/images/20210503181001.png)

![按站点和指标类型配置的告警条件](https://blog.poetries.top/img/static/images/20210503181010.png)

![告警通知渠道与接收人配置](https://blog.poetries.top/img/static/images/20210503181017.png)

![告警触发记录列表](https://blog.poetries.top/img/static/images/20210503181025.png)
...
![监控系统首页 Dashboard 效果](https://blog.poetries.top/img/static/images/20210503181035.png)
...
- [ ] SDK 全部逻辑套在 `try catch` 里，自身报错不影响宿主页面
- [ ] SDK 体积控制住，同步引入的话尽量压到几 KB 量级，别为了监控拖慢首屏
- [ ] `window.onerror`、`unhandledrejection`、资源 `error`（捕获阶段）三条线都接上
- [ ] `XMLHttpRequest` 和 `fetch` 都做了劫持，劫持后原样调用原实现
- [ ] 跨域脚本加了 `crossorigin`，CDN 配了对应的 CORS 响应头
- [ ] 抽样率支持远程下发，拉不到配置有内置默认值兜底
- [ ] 有忽略名单，能过滤浏览器插件和第三方脚本的噪音错误
- [ ] 上报数据做过脱敏，接口参数里不含手机号、token 这类敏感信息
- [ ] 页面卸载时的上报走 `sendBeacon`，判断了返回值并有降级路径
...
- [ ] `.map` 文件只上传到内网监控后台，没有发到公网 CDN
- [ ] `.map` 按版本号或构建哈希归档，能和线上产物一一对应
...
- [Ajax-hook 源码](https://github.com/wendux/Ajax-hook)
